...
public class RecyclerViewFastScroller extends LinearLayout {
    ...
    public interface BubbleTextGetter {
        String getTextToShowInBubble(double pos);
    }
    ...
    private boolean actionScrollbar(MotionEvent event) {
    ...
    private void setRecyclerViewPosition(float y) {
        if (recyclerView != null) {
            final double itemCount = recyclerView.getAdapter().getItemCount();
            float proportion;
            if (handle.getY() == 0)
                proportion = 0f;
            ...
            final double targetPos = getValueInRange(0, itemCount - 1, (int) (proportion * (float) itemCount));
            ((LinearLayoutManager) recyclerView.getLayoutManager()).scrollToPositionWithOffset((int) targetPos, 0);
    ...
    private double getValueInRange(int min, double max, double value) {
    ...
    private void setBubbleAndHandlePosition(float y) {
        final double handleHeight = handle.getHeight();
        handle.setY((float) getValueInRange(0, height - handleHeight, (y - handleHeight / 2)));
        if (bubble != null) {
            int bubbleHeight = bubble.getHeight();
            bubble.setY((float) getValueInRange(0, height - bubbleHeight - handleHeight / 2, (int) (y - bubbleHeight)));
        }
    }
